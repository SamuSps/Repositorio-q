...
# Asegura que el directorio 'src' esté en la ruta para importar módulos
...
@pytest.fixture
def df_prueba():
    """Crea un DataFrame de prueba con columnas numéricas."""
    return pd.DataFrame({
        "A": [1, 2, 3, 4, 5],
        "B": [5, 4, 3, 2, 1],
        "C": [10, 20, 30, 40, 50]
...
def test_columna_separacion(df_prueba, test_size):
    """
    Verifica que train_test_split separe los datos correctamente
    según el tamaño de prueba (test_size) y que no haya solapamiento de filas.
    """
    features = ["A", "B"]
    target = "C"

    # 1. Preprocesamiento (ejecuta la función bajo prueba o dependencia)
    ...
    # --- Verificar suma de filas (la suma debe ser igual al total) ---
    assert len(X_train) + len(X_test) == len(df_proc)
    assert len(y_train) + len(y_test) == len(df_proc)
    ...
    # Se usa pytest.approx debido al redondeo en datasets pequeños
    assert actual_test_ratio == pytest.approx(test_size, rel=0.5)
    assert actual_train_ratio == pytest.approx(1 - test_size, rel=0.5)

    # --- Verificar no solapamiento de valores (asumiendo valores únicos en este df_prueba) ---
    for col in features:
        # Los conjuntos de valores en train y test no deben tener elementos en común
...
def test_dataset_pequeno():
    """Prueba la separación en un DataFrame con muy pocas filas."""
    ...
    try:
        ...
        pass  # Es aceptable para datasets muy pequeños que la división falle o sea trivial

# --- Test: manejo de columnas inexistentes ---
def test_columnas_inexistentes(df_prueba):
    """Verifica que preprocesar_datos lance un error si se le dan columnas inexistentes."""
...
def test_preprocesamiento_constante(df_prueba):
    """Verifica el llenado de valores nulos (None) con un valor constante."""
...
def test_preprocesamiento_media_mediana(df_prueba):
    """Verifica el llenado de valores nulos con la media y la mediana de la columna."""
    df_nan = df_prueba.copy()
    df_nan.loc[0, "A"] = None
    df_nan.loc[1, "B"] = None
    ...
    # Verificar que no quedan NaNs después de la imputación
